...
\begin{frame} % No title at first slide
    ...
    \sectionmeta{
        출제자: 노태윤(\idtext{areian13})\\
        태그: \tagtext{\#set, \#mst, \#greedy}\\
        예상 난이도: \tiertext{Gold2}
    }
    \begin{itemize}
...
\begin{frame}{\textbf{\pid}. \pname}
    \begin{itemize}
        ...
        \item 해시맵을 쓰기 위해선 키 타입에 대해 \texttt{hash} 함수와 \texttt{equal} 연산자(==)가 정의 돼있어야 합니다.
        하지만 C\texttt{++}에선 pair의 \texttt{hash} 함수가 정의 돼있지 않습니다. 따라서 pair의 \texttt{hash} 함수를 직접 정의해주셔야 합니다.\\
        언어별 정의 방법은 구글링 해보는 것을 추천합니다

        \item 트리맵은 키 타입에 대해 \texttt{less} 연산자(<)만 정의 돼있으면 됩니다.\\
        C\texttt{++}는 pair에 대해 \texttt{less} 연산자가 정의돼 있으므로 별도의 구현 없이 바로 사용할 수 있습니다.
    \end{itemize}
\end{frame}
%----------------------------------------------------------------------------
\begin{frame}{\textbf{\pid}. \pname}
